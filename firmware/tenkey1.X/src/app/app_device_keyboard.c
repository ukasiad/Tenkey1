...
// 入出力ピン数
#define OUTPUT_PIN_MAX  5
#define INPUT_PIN_MAX   4

// チャタリング抑制用閾値。1あたりの時間は謎。
#define CHATTAERING_THRESHOLD   50

#define REPORT_BUFF_SIZE 6

// キーコードテーブル
unsigned char KEY_TABLE[OUTPUT_PIN_MAX][INPUT_PIN_MAX] = {
                //  INPUT0  INPUT1  INPUT2  INPUT3
                //  numlock tab     /       *
/* OUTPUT0 */   {   0x53,   0x2B,   0x54,   0x55    },
                //  7       8       9       -
/* OUTPUT1 */   {   0x5F,   0x60,   0x61,   0x56    }, // {   0x5F,   0x60,   0x61,   0x56    },
                //  4       5       6       +
/* OUTPUT2 */   {   0x5C,   0x5D,   0x5E,   0x57    }, // {   0x5C,   0x5D,   0x5E,   0x57    },
                //  1       2       3       enter
/* OUTPUT3 */   {   0x59,   0x5A,   0x5B,   0x58    }, // {   0x59,   0x5A,   0x5B,   0x58    },
                //  0       .
/* OUTPUT4 */   {   0x62,   0x63,   0x00,   0x00    }, // {   0x62,   0x63,   0x00,   0x00    }
};
...
typedef struct
{
    ...
    unsigned char keys[REPORT_BUFF_SIZE];
} KEYBOARD;
...
// キースキャン用バッファ
unsigned short scanBuff[OUTPUT_PIN_MAX][INPUT_PIN_MAX];
...
void APP_KeyboardTasks(void)
{
    ...
    if(HIDTxHandleBusy(keyboard.lastINTransmission) == false)
    {
        ...
        memcpy(inputReport.keys, keyboard.keys, REPORT_BUFF_SIZE);
...
void keyscan() {
    memset(keyboard.keys, 0, sizeof (keyboard.keys));
    int reportIndex = 0;
    for (int i = 0; i < OUTPUT_PIN_MAX; i++) {
        // high
        myKbPinOut(i, 1);
        for (int j = 0; j < INPUT_PIN_MAX; j++) {
            if (myKbChkIn(j)) {
                if (scanBuff[i][j] <= CHATTAERING_THRESHOLD) {
                    scanBuff[i][j]++;
                }
            } else {
                if (scanBuff[i][j] > 0) {
            ...
            if (scanBuff[i][j] > CHATTAERING_THRESHOLD) {
                if (KEY_TABLE[i][j] != 0x00) {
                    keyboard.keys[reportIndex] = KEY_TABLE[i][j];
                    reportIndex++;
                }
            }
...
void myKbPinOut(unsigned char pinNo, unsigned char onoff) {
    switch (pinNo) {
        case 0: LATCbits.LATC7 = onoff; break;
        case 1: LATCbits.LATC6 = onoff; break;
        case 2: LATCbits.LATC5 = onoff; break;
        case 3: LATCbits.LATC4 = onoff; break;
        case 4: LATCbits.LATC3 = onoff; break;
    }
}

unsigned char myKbChkIn(unsigned char pinNo) {
    switch (pinNo) {
        case 0: return PORTBbits.RB4;
        case 1: return PORTBbits.RB5;
        case 2: return PORTBbits.RB6;
        case 3: return PORTBbits.RB7;
    }
    return 0;
}
